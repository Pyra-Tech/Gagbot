const { getDressProtocolOutfit } = require("../../functions/getters/config/getDressProtocolOutfit.js");
...
let dressprotocoltick = async (serverID, userID, heavy) => {
    if (!heavy.dressprotocol) {
        ...
        if (heavy.dressprotocol.stage >= 4) {
            removeHeavy(serverID, userID, 'wardrobe_device')
            return;
        }
...
let tick = async (serverID, userID, datain) => {
    let thisheavyrestraint = getHeavy(serverID, userID, 'wardrobe_device');
