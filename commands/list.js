...
async function generateList(listchoice, page, details) {
    ...
    let buttons = [
        // Page Down
        new ButtonBuilder().setCustomId(`list_pagedown_${listchoice}_${page}_${details}`).setLabel("⮜ Prev Page").setStyle(ButtonStyle.Secondary).setDisabled(page <= 1),
        // Current Page
        ...
        // Page Up
        new ButtonBuilder().setCustomId(`list_pageup_${listchoice}_${page}_${details}`).setLabel("Next Page ⮞").setStyle(ButtonStyle.Secondary).setDisabled(page >= maxpages)
    ]
    ...
    let menupageoptions = new StringSelectMenuBuilder()
        .setCustomId(`list_menuselector_${details}`)
