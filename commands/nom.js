...
module.exports = {
	data: new SlashCommandBuilder()
		.setName("nom")
		.setDescription("Attempt to nibble on someone")
        .addUserOption((opt) => opt.setName("user").setDescription("Who to nom?")),
	async execute(interaction) {
		try {
            ...
            await handleTouchEvent(interaction.guildId, interaction.user, targetuser, "nom").then(
                async (success) => {
                    await interaction.followUp({ content: `Nomming ${targetuser}`, flags: MessageFlags.Ephemeral })
                    data.nom = true;
                    ...
                    // If they're gagged or wearing a mouth covering mask, obviously they can't nom. 
                    if (!getHeadwearRestrictions(interaction.guildId, interaction.user.id).canGag) {
                        data.masked = true;
                    }
                    else if (getGag(interaction.guildId, interaction.user.id)) {
                        data.gagged = true;
                    }
                    else if (!getHeavyRestrictions(interaction.guildId, interaction.user.id).touchself && data.self) {
                        data.heavy = true;
                    }
                    else {
                        data.free = true;
                ...
                async (reject) => {
                    let nomessage = `${targetuser} rejected the nom.`;
                    if (reject == "Error") {
