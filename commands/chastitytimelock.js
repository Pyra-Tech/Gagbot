...
module.exports = {
	async modalexecute(interaction) {
		...
		if (keyholderAfter == 0 && rollKeyFumble(interaction.user.id, wearer)) {
			interaction.reply({ content: "you are too frustrated to use the unlock action", flags: MessageFlags.Ephemeral })
			return
		}
	...
	componentHandlers: [
		{
			...
			async handle(interaction, wearer, keyholder, unlockTime, access, keyholderAfter) {
				if (getChastityKeyholder(wearer) != wearer && getChastityKeyholder(wearer) != keyholder) {
					interaction.reply({ content: "Keyholder has changed since start of timelocking", flags: MessageFlags.Ephemeral })
					return
				}
				...
				if (timelockChastity(interaction.client, wearer, keyholder, Math.floor(modifiedUnlockTime), Number(access), Number(keyholderAfter), interaction.channel.id)) {
					...
					interaction.reply({ content: "Timelock confirmed", flags: MessageFlags.Ephemeral })
				} else {
					interaction.reply({ content: "Failed to timelock chastity belt", flags: MessageFlags.Ephemeral })
				}
			},
		},
	],
